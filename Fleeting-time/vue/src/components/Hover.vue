...
<template>
  ...
  const containerClass = computed(() =>
    cn(
      "group/card relative h-60 w-60 overflow-hidden rounded-2xl bg-transparent md:h-96 md:w-96",
      props.class,
    ),
  );
  ...
  const childrenClass = computed(() =>
    cn(
      "absolute bottom-4 left-4 z-40 text-blue transition-opacity duration-300",
      props.childrenClass,
    ),
  );
